...
def exportar_clientes(df, carpeta='02.descargable'):
    formatos = {
        'CSV': lambda: df_clients.to_csv(f'{carpeta}/CSV/01.CSV correctos/clientes.csv', index=False, encoding='utf-8-sig'),
        'JSON': lambda: df_clients.to_json(f'{carpeta}/JSON/01.JSON correctos/clientes.json', orient='records', lines=True, force_ascii=False),
        'JSON_EXCEL': lambda: df_clients.to_json(f'{carpeta}/JSON para excel/01.JSON para excel correctos/clientes.json', orient='table'),
        'SQL': lambda: exportar_sql(df_clients, f'{carpeta}/SQL/01.SQL correctos/clientes.sql', 'clientes'),
        'PARQUET': lambda: df_clients.to_parquet(f'{carpeta}/PARQUET/01.PARQUET correctos/clientes.parquet', index=False),
        'FEATHER': lambda: df_clients.to_feather(f'{carpeta}/FEATHER/01.FEATHER correctos/clientes.feather'),
        'EXCEL': lambda: df_clients.to_excel(f'{carpeta}/XLSX/01.XLSX correctos/clientes.xlsx', index=False)
    }
