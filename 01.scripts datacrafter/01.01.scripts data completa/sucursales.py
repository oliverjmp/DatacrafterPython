...
def exportar_branches(df, carpeta='02.descargable'):
    formatos = {
        'CSV': lambda: df_branches.to_csv(f'{carpeta}/CSV/01.CSV correctos/sucursales.csv', index=False, encoding='utf-8-sig'),
        'JSON': lambda: df_branches.to_json(f'{carpeta}/JSON/01.JSON correctos/sucursales.json', orient='records', lines=True, force_ascii=False),
        'JSON_EXCEL': lambda: df_branches.to_json(f'{carpeta}/JSON para excel/01.JSON para excel correctos/sucursales.json', orient='table'),
        'SQL': lambda: exportar_sql(df_branches, f'{carpeta}/SQL/01.SQL correctos/sucursales.sql', 'sucursales'),
        'PARQUET': lambda: df_branches.to_parquet(f'{carpeta}/PARQUET/01.PARQUET correctos/sucursales.parquet', index=False),
        'FEATHER': lambda: df_branches.to_feather(f'{carpeta}/FEATHER/01.FEATHER correctos/sucursales.feather'),
        'EXCEL': lambda: df_branches.to_excel(f'{carpeta}/XLSX/01.XLSX correctos/sucursales.xlsx', index=False)
    }
