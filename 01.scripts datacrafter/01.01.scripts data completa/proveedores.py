...
def exportar_providers(df, carpeta='02.descargable'):
    formatos = {
        'CSV': lambda: df_providers.to_csv(f'{carpeta}/CSV/01.CSV correctos/proveedores.csv', index=False, encoding='utf-8-sig'),
        'JSON': lambda: df_providers.to_json(f'{carpeta}/JSON/01.JSON correctos/proveedores.json', orient='records', lines=True, force_ascii=False),
        'JSON_EXCEL': lambda: df_providers.to_json(f'{carpeta}/JSON para excel/01.JSON para excel correctos/proveedores.json', orient='table'),
        'SQL': lambda: exportar_sql(df_providers, f'{carpeta}/SQL/01.SQL correctos/proveedores.sql', 'proveedores'),
        'PARQUET': lambda: df_providers.to_parquet(f'{carpeta}/PARQUET/01.PARQUET correctos/proveedores.parquet', index=False),
        'FEATHER': lambda: df_providers.to_feather(f'{carpeta}/FEATHER/01.FEATHER correctos/proveedores.feather'),
        'EXCEL': lambda: df_providers.to_excel(f'{carpeta}/XLSX/01.XLSX correctos/proveedores.xlsx', index=False)
    }
...
def exportar_sql(df, ruta, nombre_tabla):
    with open(ruta, 'w', encoding='utf-8') as f:
        ...
        f.write("    provider_id VARCHAR(8) PRIMARY KEY,\n")
        f.write("    name VARCHAR(100),\n")
        f.write("    nif VARCHAR(15),\n")
